    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          <div className="w-20 h-20 bg-gradient-to-r from-blue-600 to-blue-700 rounded-full flex items-center justify-center mb-6 shadow-lg float">
            <FileText className="w-10 h-10 text-white" />
          </div>
          <div className="w-12 h-12 border-4 border-blue-200 border-t-blue-600 rounded-full animate-spin mx-auto mb-4"></div>
          <h2 className="text-xl font-semibold text-gray-800 mb-2 fade-in">Loading Your Guidance</h2>
          <p className="text-gray-600 slide-up animate-delay-200">Preparing your personalized application pathway...</p>
        </div>
      </div>
    )
  }
    ...
    <div className="min-h-screen bg-blue-50 py-12">
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12">
          ...
          <h1 className="text-4xl md:text-5xl font-bold text-gray-800 mb-4">Your Complete Pathway</h1>
          <p className="text-xl text-gray-600 max-w-2xl mx-auto">Step-by-step guide to complete your application successfully</p>
        </div>

        {/* Timeline Overview */}
        {timeline.estimated_days && (
          <div className="card bg-gradient-to-r from-blue-50 to-purple-50 mb-6 scale-in animate-delay-100 hover-lift">
            <div className="flex items-center justify-between">
              ...
              <Clock className="w-12 h-12 text-blue-600 float" />
            </div>
          </div>
        )}

        {/* Navigation Tabs */}
        <div className="flex gap-2 mb-6 overflow-x-auto slide-up animate-delay-200">
          {sections.map(({ id, label, icon: Icon }, index) => (
            <button
              key={id}
              onClick={() => setActiveSection(id)}
              className={`flex items-center gap-2 px-4 py-3 rounded-lg font-medium transition-all duration-300 whitespace-nowrap hover-lift scale-in animate-delay-${(index + 3) * 100} ${
                activeSection === id
                  ? 'bg-blue-600 text-white shadow-lg pulse-glow'
                  : 'bg-white text-gray-700 hover:bg-blue-50 hover:text-blue-700'
              }`}
            >
              <Icon className={`w-5 h-5 transition-transform duration-300 ${activeSection === id ? 'rotate-in' : ''}`} />
              {label}
            </button>
          ))}
        </div>
        ...
        <div className="space-y-6">
          ...
          {activeSection === 'pre-application' && (
            <div className="card slide-in-left animate-delay-300">
              <h2 className="text-2xl font-semibold text-gray-800 mb-4 flex items-center gap-2">
                <Clock className="w-6 h-6 text-blue-600 bounce-in" />
                Pre-Application Steps
              </h2>
              <div className="space-y-4">
                {pre_application_steps.length > 0 ? (
                  pre_application_steps.map((step, index) => (
                    <div key={index} className={`flex gap-4 hover-lift scale-in animate-delay-${(index + 4) * 100}`}>
                      <div className="flex-shrink-0">
                        <div className="w-8 h-8 bg-blue-600 text-white rounded-full flex items-center justify-center font-semibold bounce-in animate-delay-400">
                          {index + 1}
                        </div>
                      </div>
                      <div className="flex-1">
                        <h3 className="font-semibold text-gray-800 mb-1 slide-up animate-delay-500">{step.title}</h3>
                        <p className="text-gray-700 slide-up animate-delay-600">{step.description}</p>
                        {step.requirements && (
                          <ul className="list-disc list-inside mt-2 text-sm text-gray-600 slide-up animate-delay-700">
                            {step.requirements.map((req, i) => (
                              <li key={i} className={`slide-up animate-delay-${(i + 8) * 100}`}>{req}</li>
                            ))}
                          </ul>
                        )}
                      </div>
                  ...
                  <p className="text-gray-600 slide-up animate-delay-400">No pre-application steps required.</p>
                )}
              </div>
            </div>
          )}
